...
describe('queryBuilder', () => {
  ...
  test('returns a valid query with default offset', () => {
    ...
    expect(builtQuery).toBe(
      'https://api.giphy.com/v1/gifs/search?api_key=z3TCxWMXI3poet0DNQBeC8RfYrprX7U1&q=kittens&offset=0&limit=5&rating=G&lang=en'
    );
  });
  ...
  test('returns a valid query with custom offset', () => {
    ...
    expect(builtQuery).toBe(
      'https://api.giphy.com/v1/gifs/search?api_key=z3TCxWMXI3poet0DNQBeC8RfYrprX7U1&q=kittens&offset=1&limit=5&rating=G&lang=en'
    );
  });
});
